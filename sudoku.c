...
void writehs(short n, int score, struct term_states term_states);
void clear(void);

int main(void) {
  short A[9][9], n, q, p0 = 1;
  struct term_states term_states;
  tcgetattr(STDIN_FILENO, &term_states.reset);
  term_states.raw = term_states.reset;
  term_states.input = term_states.reset;
  term_states.raw.c_lflag &= ~(ECHO | ICANON | ISIG);
  term_states.input.c_lflag &= ~(ISIG);
  tcsetattr(STDIN_FILENO, TCSAFLUSH, &term_states.raw);
  ...
  mainmenu:
    ...
    newgame:
      ...
      writehs(q, (int)ttaken, term_states);
      break;
    case 2:
      respuz(A, 0);
      while (edit(A, 0, &x, &y)) {
  ...
  fflush(stdout);
  clear();
  tcsetattr(STDIN_FILENO, TCSAFLUSH, &term_states.reset);
  return 0;
}
...
void writehs(short n, int score, struct term_states term_states) {
  ...
  for (i = 0; i < 5; i++) {
    if (d[n].score[i] > score) {
      ...
      tcsetattr(STDIN_FILENO, TCSAFLUSH, &term_states.input);
